...
import {Client, URLEncodedFormParam, DeleteMapping, PatchMapping, PostMapping, PutMapping, HeaderParam} from "../src";
...
describe("url-encoded form param tests", () => {
  ...
  it("content type is not overridden when using HeaderParam", async () => {
    ...
    class MappingTest {
      @PostMapping({value: "/test/post", produces: "text/plain"}) // @ts-expect-error function implementation done by @Mapping decorator
      basicPost(@HeaderParam("content-type") contentType: string, @URLEncodedFormParam("id") id: string, @URLEncodedFormParam("id2") id2: string): Promise<{ hello: "string" }> {
      };
    }
